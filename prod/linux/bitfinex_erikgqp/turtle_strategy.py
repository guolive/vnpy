...
class TurtletoStrategy(CtaTemplate):
    """"""
    author = "Guo"
    ...
    variables = ["entry_up", "entry_down", "exit_up", "exit_down", "atr_value", "intra_trade_low", "intra_trade_high"]
    ...
    def on_trade(self, trade: TradeData):
        ...
        if trade.direction == Direction.LONG:
            ...
            # self.long_stop = self.long_entry - 2 * self.atr_value  # 原始止损
            self.long_stop = self.long_entry - 2 * self.atr_value
            self.write_log("止损卖出{}".format(self.long_stop))
        else:
